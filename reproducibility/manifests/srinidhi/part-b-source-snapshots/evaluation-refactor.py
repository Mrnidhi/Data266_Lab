...
def _train_one(name, cfg, datasets, vocabulary, output, device, fingerprint, resume, accepted_fingerprints=None):
    ...
    if cfg.get("validation_only", False):
        metrics = {"model_name": name, "selected_epoch": best["epoch"],
                   "selected_validation_macro_f1": best["best_f1"],
                   "parameter_count": sum(p.numel() for p in model.parameters()),
                   "train_seconds": sum(row["train_seconds"] for row in history),
                   "peak_cuda_memory_bytes": int(torch.cuda.max_memory_allocated(device)) if cuda else None,
                   "completed_training_steps": global_step, "test_evaluated": False}
        _dump(model_dir / "validation_selection.json", metrics)
        del model, optimizer
        if cuda:
            torch.cuda.empty_cache()
